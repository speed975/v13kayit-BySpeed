...
const {SpeedDatabase} = require('../Functions/SpeedDatabase');
module.exports = {
  name: "isim",
  aliases: ["i", "name", "kayıt", "k"],
  ...
  function embed(msg) {
    ...
    let collector = await msg.createButtonCollector(filter, { time: 30000 })
      ...
      if(button.id === "MAN") {
      await SpeedDatabase.man(user, message.author)
      await SpeedDatabase.setusername(user, name_2, `<@&${settings.ManRole}>`)
      await button.think(true)
      await button.reply.edit(`${user} adlı kullanıcı başarıyla <@&${settings.ManRole}> rolüyle kayıt edildi`)
      }
      if(button.id === "WOMAN") {
      await SpeedDatabase.woman(user, message.author)
      await SpeedDatabase.setusername(user, name_2, `<@&${settings.WomanRole}>`)
      await button.think(true)
    ...
    await user.setNickname(name_2)
    SpeedDatabase.setusername(user, name_2, "İsim Değiştirme")
